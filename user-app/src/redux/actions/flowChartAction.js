...
export const loadComponents = (id) => async (dispatch) => {
    dispatch(setLoading(SET_FLOWCHART_STATUS, 'loadComponents'));
    
    try{
        const res = await axios.get(`/api/flowchart/load-components/?problem_id=${id}`);
        
        dispatch({
            type: SET_POSITION,
            payload: res.data.res.position
        });

        dispatch({
            type: SET_COMPONENTS,
            payload: res.data.res.components
        });

        dispatch(setFinish(SET_FLOWCHART_STATUS, 'loadComponents'));
    }
    catch(err){
        console.log(err.response.data);
        dispatch(setErrors(SET_FLOWCHART_STATUS, 'loadComponents', err.response.data));
    }
}
